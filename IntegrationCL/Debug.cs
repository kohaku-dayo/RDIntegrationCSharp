...
namespace UniEx
{
    ...
    [Obsolete("機能が上手く動作しない可能性があります。")]
    public static class Analyze_Extend
    {
        [Obsolete("深度1以上を探索できないため、使い勝手がよくないです。")]
        public static (int line, string file, string name, string arg) Analyze<T>(
            this T target,
            [CallerLineNumber] int line = 0,
            [CallerFilePath] string file = null,
            [CallerMemberName] string name = null,
            [CallerArgumentExpression("target")] string arg = null
            )
        {
            return (line, file, name, arg);
        }
        [Obsolete("実装過程メソッドです。使用しないでください。")]
        static StackFrame Analyze2<T>(this T target, int num) => new StackFrame(num);
    }
